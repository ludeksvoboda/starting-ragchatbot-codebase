...
const debugMode = true; // Enable detailed logging
...
async function testApiConnectivity() {
  ...
  try {
    ...
    if (error.name === 'TypeError' && error.message.includes('fetch')) {
      errorLog(
        'DIAGNOSIS: Network request failed - server may not be running or CORS issue'
      );
    } else if (error.name === 'AbortError') {
      errorLog(
        'DIAGNOSIS: Request was aborted - timeout or network interruption'
      );
    } else {
...
async function sendMessage() {
  ...
  try {
    ...
    debugLog(
      `Request completed in ${requestDuration}ms with status: ${response.status}`
    );
    ...
    if (
      ...
      userErrorMessage =
        'Network Error: Cannot connect to server. Please check if the server is running on http://localhost:8000';
      errorLog(
        'DIAGNOSIS: Failed to fetch - likely server not running or network issue'
      );
    } else if (error.name === 'SyntaxError' && error.message.includes('JSON')) {
      userErrorMessage = 'Error: Server returned invalid response format';
      errorLog(
        'DIAGNOSIS: JSON parsing error - server returned non-JSON response'
      );
    } else if (error.name === 'AbortError') {
...
function addMessage(content, type, sources = null, isWelcome = false) {
  debugLog(
    `Adding message: type=${type}, contentLength=${content.length}, sourcesCount=${sources ? sources.length : 0}`
  );
...
async function loadCourseStats() {
  ...
  try {
    ...
    if (courseTitles) {
      if (data.course_titles && data.course_titles.length > 0) {
        courseTitles.innerHTML = data.course_titles
          .map(
            title => `<div class="course-title-item">${escapeHtml(title)}</div>`
          )
          .join('');
    ...
    if (error.message.includes('Failed to fetch')) {
      errorLog(
        'DIAGNOSIS: Cannot connect to courses API - server may not be running'
      );
    }
  }
}
